...
import { GVL } from "../../src/gvl/GVL";
import { Vendor } from "../../src/gvl/model/Vendor";
import { IntMap } from "../../src/gvl/model/IntMap";
...
describe("GVL", (): void => {
  ...
  it("should produce a clone with the same language as the original gvl using its language cache", async (): Promise<void> => {
    const fetchStub = sinon.stub(JsonHttpClient, "fetch");
    const gvl: GVL = new GVL(vendorlistJson);
      ...
      Object.keys(gvl[gvlKey]).forEach((id: string): void => {
        ...
        Object.keys(vendorlistJson.vendors).forEach((vendorId: string): void => {
          ...
          if (values && values.includes(intId)) {
            expect(gvlMap[vendorId], `vendorId ${vendorId} should be in the map when ${gvlMethodName}() is called`).to
              .not.be.undefined;
