import { GvlClient } from "./client/GvlClient";
import { GvlError } from "./error/GvlError";
import { ByPurposeVendorMap, Declarations, Feature, IDSetMap, Purpose, Stack, Vendor, VendorList } from "./model";
import { Cloneable } from "./model/Cloneable";
import { ConsentLanguages } from "./model/ConsentLanguages";
...
export class Gvl extends Cloneable<Gvl> implements VendorList {
  ...
  private async fetchJson(url: string): Promise<void | Error> {
    try {
      this.populate((await GvlClient.fetch(url)) as VendorList);
    } catch (err) {
